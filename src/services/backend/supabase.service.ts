...
function isMissingCreateAreaRpc(error: unknown): boolean {
  const msg = errorMessage(error).toLowerCase();
  return msg.includes("create_area") && (
    msg.includes("schema cache") ||
    msg.includes("could not find the function") ||
    msg.includes("function public.create_area") ||
...
function logErr(op: string, error: unknown): never {
  ...
  logger.error("supabase", `${op}: ${normalized.message}`);
  throw normalized;
}
...
export async function fetchAreas(): Promise<Area[]> {
  const { data, error } = await supabase
    ...
    .select("*")
    .order("sort_order");
  if (error) logErr("fetchAreas", error);
  return data ?? [];
}
...
export async function createArea(name: string, color = "#6B7280"): Promise<Area> {
  ...
  const { data, error } = await supabase
    .rpc("create_area", { p_name: trimmedName, p_color: color })
    .single();
  if (!error) return data as Area;

  // Keep first-space capture working even if the RPC migration is not deployed yet.
  if (isMissingCreateAreaRpc(error)) {
    logger.warn("supabase", "createArea: create_area RPC unavailable, falling back to direct insert");
    const user_id = await getCurrentUserId();
    const { data: fallback, error: fallbackError } = await supabase
      .from("areas")
      .insert({ name: trimmedName, color, user_id })
      .select()
      .single();
    if (fallbackError) logErr("createArea(fallback)", fallbackError);
    return fallback;
  }

  logErr("createArea", error);
  return data as Area;
}
...
export async function createProject(
  ...
  const { data, error } = await supabase
    .from("projects")
    .insert({ name: trimmedName, area_id: resolvedAreaId, color, user_id })
    .select()
    .single();
  if (error) logErr("createProject", error);
  return data;
}
