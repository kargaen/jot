...
import { filterVisibleProjects, filterVisibleTasks, loadHiddenAreas, saveHiddenAreas } from "../lib/tasks";
import {
...
function friendlyDue(dueDate: string | null, dueTime: string | null) {
...
function friendlyRecurrence(rule: string | null) {
  if (!rule) return null;
...
function sortTasks(tasks: TaskWithTags[]) {
...
function countByProject(tasks: TaskWithTags[]) {
...
function CaptureComposer({
  ...
  async function handleCreate() {
    ...
    setError(null);
    try {
      let resolvedProjectId = parsed.project?.id ?? null;
      if (!resolvedProjectId && parsed.suggestedProjectName) {
        const createdProject = await createProject(parsed.suggestedProjectName);
      ...
      const task = await createTask({
        ...
        icon: suggestIcon(parsed.title.trim()),
...
export default function MobileApp({ launchNotice = null }: { launchNotice?: string | null }) {
  ...
  const projectTaskCount = useMemo(() => countByProject(visibleTasks), [visibleTasks]);
